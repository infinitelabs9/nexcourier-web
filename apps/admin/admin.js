(function startNexCourierAdmin(global) {
  ...
  function renderDashboard() {
    const metricCards = data.metrics.map(function metricCard(metric) {
      return '<button class="metric-card" type="button" data-route="' + esc(metric.page) + '" data-tone="' + esc(metric.tone) + '"><span>' + esc(metric.label) + "</span><strong>" + esc(metric.value) + "</strong><small>" + esc(metric.note) + "</small></button>";
    }).join("");
    ...
    return pageHead("Buenos días", "Operación — Centro de recepción internacional · Demo data", routeButton("Registrar recepción", "reception", null, "button--primary")) +
      '<section class="metric-grid" aria-label="Indicadores operativos">' + metricCards + "</section>" +
      '<section class="dashboard-grid"><div class="panel"><div class="panel-head"><div><h2>Prioridades</h2><p>Trabajo que requiere atención hoy.</p></div></div><div class="priority-list">' + priorities + '</div></div><aside class="shipment-spotlight"><span class="spotlight-label">Próximo envío</span><h2>' + esc(shipment.shipmentId) + '</h2><p class="spotlight-route">' + esc(shipment.route) + '</p><div class="route-line"></div><div class="spotlight-stats"><span><small>Paquetes</small><strong>' + esc(shipment.packageCount) + '</strong></span><span><small>Peso</small><strong>' + esc(shipment.grossWeight) + '</strong></span><span><small>Salida</small><strong>18 Sep · 21:30</strong></span></div>' + routeButton("Ver envío", "shipment", shipment.shipmentId, "button--primary") + '</aside><div class="panel activity-panel"><div class="panel-head"><div><h2>Actividad reciente</h2><p>Últimos movimientos en la operación.</p></div></div><div class="activity-list">' + activity + "</div></div></section>";
  }
  ...
  function renderUnidentified() {
    const cards = data.unidentified.map(function unidentifiedCard(item) {
      const match = item.possibleCustomerId ? '<div class="possible-match"><span>Posible coincidencia</span><strong>' + esc(item.possibleCustomerId) + ' · ' + esc(item.possibleCustomerName) + '</strong><ul class="evidence-list">' + item.evidence.map(function evidence(reason) { return "<li>" + esc(reason) + "</li>"; }).join("") + '</ul>' + badge("Requiere confirmación", "critical") + '</div>' : '<div class="notice-box notice-box--warning"><strong>Sin coincidencias suficientes</strong><span>Buscar por etiqueta, comercio o datos del casillero.</span></div>';
      return '<article class="unidentified-card"><div><span class="demo-label">' + esc(item.scenario) + '</span><h3>' + esc(item.tracking) + '</h3><p>' + esc(item.caseId) + '</p></div><div class="summary-grid"><div class="summary-item"><small>Recibido</small><strong>' + esc(item.received) + '</strong></div><div class="summary-item"><small>Peso</small><strong>' + esc(item.weight) + '</strong></div><div class="summary-item"><small>Ubicación</small><strong>' + esc(item.location) + '</strong></div></div><button class="photo-placeholder" type="button" data-action="photo-demo">' + icon("file") + '<strong>Ver etiqueta</strong><span>Placeholder demo</span></button>' + match + '<div class="card-actions"><button class="button button--small button--quiet" type="button" data-action="photo-demo">Ver etiqueta</button><button class="button button--small button--quiet" type="button" data-action="customer-search-demo">Buscar cliente</button>' + (item.possibleCustomerId ? '<button class="button button--small button--navy" type="button" data-action="review-match" data-case="' + esc(item.caseId) + '">Revisar coincidencia</button>' : '<button class="button button--small button--navy" type="button" data-action="create-case">Crear caso</button>') + '</div></article>';
    }).join("");
    return pageHead("Paquetes sin identificar", "Paquetes recibidos físicamente cuyo propietario todavía no pudo confirmarse.", routeButton("Nueva recepción", "reception", null, "button--primary")) + '<div class="notice-box notice-box--warning"><strong>Control de propiedad</strong><span>Ninguna coincidencia sugerida se asigna automáticamente. Siempre requiere confirmación humana.</span></div><section class="unidentified-list">' + cards + "</section>";
  }
  ...
  function renderArrivals() {
    ...
    return pageHead("Llegadas Paraguay", "Conciliar lo esperado con lo recibido físicamente.", '<button class="button button--primary" type="button" data-action="start-arrival">Iniciar recepción</button>') + '<section class="panel"><div class="panel-head"><div><h2>' + esc(arrival.shipmentId) + '</h2><p>' + esc(arrival.route) + ' · Reconciliación de demo</p></div>' + badge("Recepción parcial", "warning") + '</div><div class="panel-body"><div class="arrival-overview">' + stats + '</div><div class="reconciliation-grid"><div class="scan-progress"><div class="scan-progress-head"><span><strong>Progreso de escaneo</strong><small>' + esc(arrival.scanned) + ' de ' + esc(arrival.expected) + '</small></span><strong>98.9%</strong></div><div class="progress-bar"><i style="width:98.9%"></i></div><div class="card-actions"><button class="button button--navy" type="button" data-action="start-arrival">Iniciar recepción</button><button class="button button--quiet" type="button" data-action="show-discrepancies">Ver discrepancias</button><button class="button button--quiet" type="button" data-route="packages">Ver paquetes</button></div></div><div><h3 class="section-title">Discrepancias detectadas</h3><ul class="discrepancy-list">' + arrival.discrepancies.map(function discrepancy(item) { return '<li><i aria-hidden="true"></i><span><strong>' + esc(item) + '</strong><small>Requiere control operativo</small></span></li>'; }).join("") + '</ul><p class="scan-hint">Datos de demostración; no representan una llegada real.</p></div></div></div></section>';
  }
  ...
  function renderTeam() {
    const rows = data.team.map(function teamRow(item) { return ['<span class="profile-summary"><span class="avatar">OD</span><strong>' + esc(item.name) + "</strong></span>", esc(item.role), esc(item.location), badge(item.status), '<button class="button button--small button--quiet" type="button" data-action="team-demo">Ver</button>']; });
    return pageHead("Equipo", "Roles y disponibilidad operativa de ejemplo.", '<button class="button button--primary" type="button" data-action="demo-soon" data-label="Gestión de equipo">Gestionar equipo</button>') + '<section class="panel">' + simpleTable(["Empleado demo", "Rol", "Ubicación", "Estado", "Acción"], rows) + "</section>";
  }
  ...
  function currentRoute() {
    ...
    const page = renderers[params.get("page")] ? params.get("page") : "dashboard";
