(function startNexCourierAdmin(global) {
  ...
  function renderDashboard() {
    const metricCards = data.metrics.map(function metricCard(metric) {
      return '<button class="metric-card" type="button" data-route="' + esc(metric.page) + '" data-tone="' + esc(metric.tone) + '"><span class="metric-label">' + esc(metric.label) + '</span><strong>' + esc(metric.value) + '</strong><small>' + esc(metric.note) + '</small><span class="metric-link">Abrir cola ' + icon("chevron") + "</span></button>";
    }).join("");
    ...
    const weeklyBars = [["Lun", 52], ["Mar", 68], ["Mié", 61], ["Jue", 78], ["Vie", 82], ["Sáb", 44], ["Hoy", 71]].map(function weeklyBar(item) {
      return '<span class="trend-column"><i style="height:' + esc(item[1]) + '%"><b>' + esc(item[1]) + '</b></i><small>' + esc(item[0]) + "</small></span>";
    }).join("");
    ...
    return pageHead("Buenos días", "Panorama operativo · Centro de recepción internacional", routeButton("Registrar recepción", "reception", null, "button--primary")) +
      ...
      '<section class="analytics-grid" aria-label="Analítica operativa"><article class="panel trend-panel"><div class="panel-head"><div><h2>Flujo de recepción</h2><p>Paquetes procesados durante los últimos siete días.</p></div><span class="analytics-total"><strong>456</strong><small>esta semana</small></span></div><div class="trend-chart" role="img" aria-label="Tendencia semanal de paquetes procesados">' + weeklyBars + '</div></article><article class="panel"><div class="panel-head"><div><h2>Distribución de colas</h2><p>Capacidad operativa utilizada por estado.</p></div></div><div class="queue-list">' + queueRows + "</div></article></section>" +
  ...
  function renderUnidentified() {
    const cards = data.unidentified.map(function unidentifiedCard(item) {
      ...
      return '<article class="unidentified-card"><header class="unidentified-card__head"><div><span class="case-label">' + esc(item.scenario) + '</span><h3>' + esc(item.tracking) + '</h3><p>' + esc(item.caseId) + '</p></div><span class="unassigned-state"><i></i>Propietario sin asignar</span></header><div class="unidentified-card__body"><div class="case-facts"><div><small>Recibido</small><strong>' + esc(item.received) + '</strong></div><div><small>Peso</small><strong>' + esc(item.weight) + '</strong></div><div><small>Ubicación</small><strong>' + esc(item.location) + '</strong></div></div><button class="label-preview" type="button" data-action="photo-demo">' + icon("file") + '<span><strong>Etiqueta del paquete</strong><small>Vista previa disponible · Demo</small></span><b>Ver</b></button>' + match + '</div><footer class="unidentified-card__actions"><span>La asignación siempre requiere revisión humana.</span><div class="card-actions"><button class="button button--small button--quiet" type="button" data-action="customer-search-demo">Buscar cliente</button>' + (item.possibleCustomerId ? '<button class="button button--small button--navy" type="button" data-action="review-match" data-case="' + esc(item.caseId) + '">Revisar coincidencia</button>' : '<button class="button button--small button--navy" type="button" data-action="create-case">Crear caso</button>') + '</div></footer></article>';
    }).join("");
